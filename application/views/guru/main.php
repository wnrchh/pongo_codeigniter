...
<script type="text/javascript">
	var datagrid = $("#datagrid").datagrid({
		url						: "<?php echo base_url() . 'guru/data'; ?>",
		primaryField			: 'id', 
		rowNumber				: true,
		...
		columns					: [
	        {field: 'nama', title: 'Nama', editable: true, sortable: true, width: 450, align: 'left', search: true},	        
  			{field: 'mata_pelajaran', title: 'Mata Pelajaran', editable: true, sortable: false, width: 140, align: 'center', search: true},
...
function images_format(rowData, rowIndex) {
	...
	function create_update_form(rowIndex) {
		$.post("<?php echo base_url() . 'guru/form'; ?>", {index : rowIndex}).done(function(data) {
	...
	function delete_action(rowIndex) {
		swal({   
			...
			var row = datagrid.getRowData(rowIndex);
			$.post("<?php echo base_url() . 'guru/delete'; ?>", {id : row.id}).done(function(data) {
				datagrid.reload();
			});
		});
	}
